...
# ──────────────────────────────────────────────
.PHONY: launch
launch: clone hosts certs up composer-install migrate-fresh-seed ## First-time launch: clone repos, add /etc/hosts entries, generate certs, start containers, install deps, run migrations, seed DB
